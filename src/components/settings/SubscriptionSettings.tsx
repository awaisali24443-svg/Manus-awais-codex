...
import { db, doc, onSnapshot, User as FirebaseUser } from '../../firebase';

export default function SubscriptionSettings({ user }: { user: FirebaseUser | null }) {
  const [subscription, setSubscription] = useState<any>(null);
  
  const plans = [
    { name: 'Starter', price: '$0', features: ['5 Agents', '100 Tasks/mo', 'Community Support'], color: 'gray' },
    { name: 'Pro', price: '$29', features: ['Unlimited Agents', '10,000 Tasks/mo', 'Priority Support', 'Custom Integrations'], color: 'black' },
    { name: 'Enterprise', price: 'Custom', features: ['Dedicated Infrastructure', 'SLA Guarantee', '24/7 Phone Support', 'On-premise Deployment'], color: 'blue' },
  ];
    ...
    const unsub = onSnapshot(doc(db, 'subscriptions', user.uid), (snap) => {
      if (snap.exists()) {
        ...
        setSubscription({
          planName: 'Starter',
          price: '$0',
          nextRenewal: '2026-05-12T00:00:00Z',
          usage: 0
        });
      }
    });
  ...
  if (!user) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-gray-400 p-8">
        ...
        <p className="text-lg font-medium">Please sign in to view subscription settings.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
      ...
      {subscription && (
        <div className="p-8 sm:p-10 bg-gray-900 rounded-2xl text-white shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <h4 className="text-3xl sm:text-4xl font-bold mb-3 tracking-tight">{subscription.planName} Plan</h4>
            <p className="text-gray-400 mb-8 sm:mb-10 max-w-md text-sm leading-relaxed">
              Your plan will automatically renew on {new Date(subscription.nextRenewal).toLocaleDateString()}. 
              You are currently using {subscription.usage} tasks this month.
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        {plans.map((plan) => (
          <div key={plan.name} className={`p-8 rounded-2xl border transition-all duration-300 relative group ${subscription?.planName === plan.name ? 'border-gray-900 bg-white shadow-xl lg:scale-105 z-10' : 'border-gray-200 bg-white/50 hover:border-gray-300 hover:bg-white'}`}>
            {plan.name === 'Pro' && (
            ...
            >
              {subscription?.planName === plan.name ? 'Active Now' : 'Switch to ' + plan.name} {subscription?.planName !== plan.name && <ArrowRight className="w-4 h-4" />}
            </button>
          </div>
        ))}
      </div>
